...
int main(int argc, char **argv)
{
	...
	while ( nbr = read(0,buf,64) ) {
		r = cyusb_bulk_transfer(h1, 0x83, buf, nbr, &transferred, timeout * 1000);
		if ( r == 0 ) {
			printf("%s", buf);
			memset(buf,'\0',64);
			continue;
